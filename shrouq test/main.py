    ...
    while True:
        ...
        if member.role == 'admin':
            ...
            if choice == '1':
                ...
                library.add_book(book,member)
                display_all_books(library)
                
            elif choice == '6':
                display_all_books(library)
                book_id = input("Enter Book ID to remove: ")
                library.remove_book(book_id,member)
                ...
                if not member:
                    ...
                    print(f"The {book.title} book is already borrowed by {member.name} for member with ID  {user_id} on {current_date}")
                else:
                    member.borrow_book(book,member)
                ...
                if not member:
                  ...
                  current_date=datetime.datetime.now().strftime("%Y-%m_%d %H:%M:%S")
                  print(f"The {book.title} book was returned by {member.name} from member with ID {user_id} on {current_date}")
                else:
